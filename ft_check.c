...
/*   Updated: 2024/01/04 13:04:32 by saazcon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_flood_fill(t_game *g, int row, int size, int curl, int start)
{
	...
	{
		g->map[curl] = 'X';
		ft_flood_fill(g, row, size, curl + 1, start);
		ft_flood_fill(g, row, size, curl - 1, start);
		ft_flood_fill(g, row, size, curl + row, start);
		ft_flood_fill(g, row, size, curl - row, start);
	}
}
...
void	ft_check_edg(t_game *g)
{
	...
	g->map[i] == 'E' || g->map[i] == 'W'))
	{
		if (i + 1 <= size && i + 1 > 0 && g->map[i + 1] == '0')
			ft_flood_fill(g, g->row, size, i + 1, i);
		if (i - 1 <= size && i - 1 > 0 && g->map[i - 1] == '0')
			ft_flood_fill(g, g->row, size, i - 1, i);
		if (i - g->row <= size && i - g->row > 0 && g->map[i + g->row] == '0')
			ft_flood_fill(g, g->row, size, i + g->row, i);
		if (i + g->row <= size && i + g->row > 0 && g->map[i - g->row] == '0')
			ft_flood_fill(g, g->row, size, i - g->row, i);
	}
}
